...
namespace ClassProject {
	...
	std::string label;
        ...
        BDD_Node() {top_var = 0; high = 0, low = 0;}
        BDD_Node(BDD_ID top_var, BDD_ID high, BDD_ID low):low(low),high(high),top_var(top_var){}

        bool operator==(const BDD_Node &anotherNode) const
        {
            return (top_var == anotherNode.top_var && high == anotherNode.high && low == anotherNode.low);
        }
    } BDD_Node;
...
{
   protected:
        std::map<std::size_t,BDD_Node> ID_Table;
        std::vector<const BDD_Node*> ID_Nodes;
...
namespace std {
    template <>
    struct hash<ClassProject::BDD_Node>
    {
        size_t operator()(const ClassProject::BDD_Node& node) const
        {
            // Compute individual hash values for two data members and combine them using XOR and bit shifting
        return ((hash<ClassProject::BDD_ID>()(node.top_var)
                    ^ (hash<ClassProject::BDD_ID>()(node.high) << 1)) >> 1)
                    ^ (hash<ClassProject::BDD_ID>()(node.low) << 1);
